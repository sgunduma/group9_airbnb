...
# Load data
data_2022_10_12 <- read_csv("/Users/meenakshinavinkumaar/Desktop/College/AI Assignment/LA_2022.10-2023-2.9/LA_2022.10-12/listings.csv")
...
ggplot(top_10_neighbourhoods, aes(x = mean_minimum_nights, y = neighbourhood)) +
  ...
  labs(title = "Mostly available by Neighbourhood from October to December in 2022",
...
#room_type
# Price prediction based on room type
# Load required libraries
library(tidyverse)
library(caTools)
...
# Assume data_2022_10_12 is your data frame

# Split data into features and target variable
feature_cols <- c('availability_365', 'minimum_nights', 'number_of_reviews', 'reviews_per_month', 'calculated_host_listings_count', 'room_type', 'neighbourhood_group')
X <- data_2022_10_12[, feature_cols]
y <- data_2022_10_12$price
...
# Create a preprocess object to impute missing values
prep_obj <- preProcess(X_train, method = c("medianImpute"))

# Impute missing values
X_train_imputed <- predict(prep_obj, X_train)

# Fit Random Forest on imputed data
rf_model <- randomForest(x = X_train_imputed, y = y_train, ntree = 500)

# Make predictions on test set
X_test_imputed <- predict(prep_obj, X_test)
y_pred <- predict(rf_model, X_test_imputed)
...
library(ggplot2)
df_results <- data.frame(Actual = y_test, Predicted = y_pred)
ggplot(df_results, aes(x = Predicted, y = Actual)) +
